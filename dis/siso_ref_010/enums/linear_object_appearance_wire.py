...
class LinearObjectAppearanceWireBitField(ctypes.Structure):
    _fields_ = [
        ...
        # (bit position 20, boolean) Describes whether the wire is buried or not,  use {@link UInt8} values for this field
        ("_isBuried", UByte, 1)
    ]

class LinearObjectAppearanceWire(ctypes.Union):
    _fields_ = [
        ("capabilities", LinearObjectAppearanceWireBitField),
        ("asbyte", UInt8)
    ]
    ...
    def set_isBuried(self, value : UInt8):
        self.capabilities._isBuried = int(value)
    def get_isBuried(self) -> UInt8:
        return self.capabilities._isBuried
    IsBuried = property(get_isBuried, set_isBuried)
